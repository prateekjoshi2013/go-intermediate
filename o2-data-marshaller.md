...
- **Go provides the csv.NewReader and csv.NewWriter functions to read a CSV file into a slice of slice of strings and to write a slice of slice of strings out to a CSV file, but there’s no way to map that data to the fields in a struct. We’re going to add this missing functionality**. 

### Implementation 


- Like other marshalers, we’ll **define a struct tag that specifies the name of a field in the data to map it to a field in a struct**:
...
- We’ll start with Marshal, writing the function and then looking at the two helper
functions it uses:

```go

    /*
        Since we can marshal a struct of any type, we need to use a parameter of type
        interface{}. This isn’t a pointer to a slice of structs, because we are only reading
        from our slice, not modifying it.
    */
    func Marshal(v interface{}) ([][]string, error) {
    ...
    /*
        This function simply loops over the fields of the reflect.Type, reads the csv tag on
        each field, appends it into a string slice, and returns the slice.
    */
...
- Our simple marshaler is now complete. Let’s see what we have to do to unmarshal:

```go

    /*
    Since we are copying data into a slice of any kind of struct, we need to use a parameter
    of type interface{}. Furthermore, because we are modifying the value stored in
    this parameter, we must pass in a pointer to a slice of structs
    */
    func Unmarshal(data [][]string, v interface{}) error {
